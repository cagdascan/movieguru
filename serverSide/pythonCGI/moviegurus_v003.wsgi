...
def application(environ, start_response):
    ...
    if querrry == 1:
        ...
        try:
            ...
            response_body += "q2ch1"
            #result02 = collection.aggregate([{"$match":{"genres":{"$in":genres}}},{"$unwind": "$genres"},{"$match":{"genres":{"$in":genres}}},{"$group":{"_id":{"name":"$name","keys":"$keywords"}, "Gcount":{"$sum":1}}},{"$match":{"Gcount":{"$gte":len(genres)}}},{"$unwind":"$_id.keys"},{"$group":{"_id":"$_id.keys", "Kcount":{"$sum":1}}},{"$sort":{"Kcount":-1}},{"$limit":100},{"$project":{"_id":1, "value":"$_id}}]) 
